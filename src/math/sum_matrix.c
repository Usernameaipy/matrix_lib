...
int sum_matrix(matrix_t *mtr_one, matrix_t *mtr_two, matrix_t **result) {
  int stat = ok;
  if ((*result)) stat = ncm_error;
  if ((!mtr_one->matrix || mtr_one->columns <= 0 || mtr_one->rows <= 0) ||
      (!mtr_two->matrix || mtr_two->columns <= 0 || mtr_two->rows <= 0))
    stat = ncm_error;
  if (mtr_one->rows != mtr_two->rows || mtr_one->columns != mtr_two->columns)
    stat = ncm_error;
  if (stat == ok) {
